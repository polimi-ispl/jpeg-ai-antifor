"""
A simple script to test state-of-the-art detectors for synthetic image detection on our dataset of standard
JPEG compressed samples.
...
from utils.data import JPEGDataset, get_transform_list
...
def main(args: argparse.Namespace):
    ...
    jpeg_info = all_data_info.loc[all_data_info['jpeg_compressed']]
    uncompressed_info = all_data_info.loc[(~all_data_info['jpeg-ai_compressed']) & (~all_data_info['jpeg_compressed'])]
    data_info = pd.concat([jpeg_info, uncompressed_info])
    if not test_all:
    ...
    save_path = os.path.join(output_dir, 'results_jpeg-standard_vs_real')
    if debug:
